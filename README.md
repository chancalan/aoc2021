...
`aoc` that, given a day and input, will
...
`aoc-bench` that does not rely on `click` but is less
...
```
# with plain poetry
poetry run aoc DAY PATH_TO_INPUT

# with poetry shell activated (or if you just installed the distribution)
aoc DAY PATH_TO_INPUT

# example
aoc 2 inputs/day02.txt
```
